...
(async function () {
  "use strict";
  if (
    document.querySelector('meta[name="application-name"]').content == "Emby"
  ) {
    // ------ configs start------
    ...
    const displayButtonOpts = {
      ...
      onclick: () => {
        ...
        document.querySelector("#displayDanmaku").children[0].innerText =
    ...
    const translateButtonOpts = {
      ...
      onclick: () => {
        ...
        document
          ...
          .setAttribute("title", chConverTtitle[window.ede.chConvert]);
        reloadDanmaku("reload");
        console.log(
          document.querySelector("#translateDanmaku").getAttribute("title")
        );
      },
    };
    ...
    const filterButtonOpts = {
      ...
      onclick: () => {
        ...
        document.querySelector("#filteringDanmaku").children[0].innerText =
          filter_icons[level];
      },
    };
    // ------ configs end------
    ...
    class EDE {
      constructor() {
        ...
        if (window.localStorage.getItem("danmakuSwitch")) {
          this.danmakuSwitch = parseInt(
            window.localStorage.getItem("danmakuSwitch")
          );
        }
        this.danmaku = null;
    ...
    async function getEpisodeInfo(is_auto = true) {
      ...
      let searchUrl =
        "https://skiree.host/api/v2/search/episodes?anime=" +
        animeName +
        "&withRelated=true";
      if (is_auto) {
    ...
    function getComments(episodeId) {
      let url =
        "https://api.9-ch.com/cors/https://skiree.host/api/v2/comment/" +
        episodeId +
